...
<html>
  <body bgcolor="#D9DFAA">
    		<table border="1" cellspacing="1" cellpadding="8" width="700"">
    			...
    			<tr>
    				<td><a href="findXscj.action?cj.id.xh=<s:property value="#xscj[0]"/>"><s:property value="#xscj[0]"/></a> </td>
    				<td><s:property value="#xscj[1]"/></td>
    				<td><s:property value="#xscj[2]"/></td>
    				<td><s:property value="#xscj[3]"/></td>
    				<td><s:property value="#xscj[4]"/></td>
    				<td><a href="deleteOneXscj.action?cj.id.xh=<s:property value="#xscj[0]"/>&cj.id.kch=<s:property value="#xscj[5]"/>" onClick="if(!confirm('确定删除该信息吗？'))return false;else return true;">删除</a></td>
    			</tr>
    			</s:iterator>
    			<tr align="left">
    		...
    		<!--<s:form action="findXscj" method="post">
				查询学生成绩：<input type="text" name="cj.id.xh"/>
